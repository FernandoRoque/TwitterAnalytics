...
(function(input, output, session) {
  ...
  userInfoData <- eventReactive(input$showUserInfo, {
    withProgress(message = "Loading application", value = 0, {
      ...
      userInfo <- cbind(ScreenName, Name, Description, 
  ...
  tweetStatisticsData <- suppressWarnings(eventReactive(input$showTweetStats, { 
    ...
    tab$weekday <- factor(tab$weekday, levels= c("Sunday", "Monday", 
    ...
    tab$weekday <- mapvalues(tab$weekday, from = c("Sunday", "Monday", "Tuesday", 
                                                   "Wednesday", "Thursday", "Friday", 
                                                   "Saturday"), to = c("Su", "M", "Tu", "W",
                                                                       "Th", "F", "Sa"))
    return(tab)
  }))
  ...
  output$countStatsClick <- renderDataTable({
    ...
    if (is.null(d)) {
      ...
      tab <- tweetStatisticsData()
      tab$weekday <- mapvalues(tab$weekday, from = c("Su", "M", "Tu", "W",
                                                     "Th", "F", "Sa"), to = c("Sunday", "Monday", "Tuesday", 
                                                                              "Wednesday", "Thursday", "Friday", 
                                                                              "Saturday"))
      join5 <- tab %>% dplyr::group_by_(input$chooseTime_Platform1, input$chooseTime_Platform2) %>% dplyr::summarise(word = paste(text, collapse = "a`"), n = n())
      join5$n <- as.numeric(join5$n)
  ...
  output$overTimeTrends <- renderDataTable({
    withProgress(message = "Application loading", value = 0, {
      incProgress(0.7, detail = "Getting tweets")
      tab <- tweetStatisticsData()
      tab$weekday <- revalue(tab$weekday, c("W" = "Wednesday", "Th" = "Thursday", "Sa" = "Saturday",
  ...
  output$downloadTweetStats <- downloadHandler(
    filename = function(){paste('StatsData', '.csv', sep = '')},
  ...
  tweetsCloudData <- eventReactive(input$showTweetCloud, {
    tweets <- userTimeline(input$twitterUser5, n = input$tweetNum5, includeRts=FALSE,
                           excludeReplies=TRUE)
    tab <- twListToDF(tweets)
  ...
  output$userTweetsCloud = renderPlot({
    withProgress(message = "Application loading", value = 0, {
      ...
      wordcloud(words = tab2$word, freq = tab2$n, min.freq = input$minWords, scale= c(3,2), rot.per = 0,
  ...
  output$userFavoritesCloud = renderPlot({
    withProgress(message = "Application loading", value = 0, {
      ...
      wordcloud(words = tab2$word, freq = tab2$n, min.freq = input$minWords2, scale= c(3,2), rot.per = 0,
                random.order=T, colors = col)
    })
  })
  ...
  output$tweetTopicCloud = renderPlot({
    withProgress(message = "Application loading", value = 0, {
      ...
      wordcloud(words = tab2$word, freq = tab2$n, min.freq = input$minWords3, scale=c(3,2), rot.per = 0,
                colors = col)
    })
  })
  ...
  userSentimentData <- eventReactive(input$showWordBarSentiment, {
    ...
    tab4 <- tab4 %>% unnest_tokens(word, text2, token = "regex", pattern = reg) %>% 
  ...
  favoriteSentimentData <- eventReactive(input$showWordBarSentiment2, {
    ...
    tab3 <- table2 %>% unnest_tokens(word, text2, token = "regex", pattern = reg) %>% 
      dplyr::filter(!word %in% stop_words$word,str_detect(word, "[a-z]"))
  ...
  output$downloadGeographic <- downloadHandler(
    filename = function(){paste('TrendData', '.csv', sep = '')},
    content = function(file){
  ...
  output$mapFollowing <- renderLeaflet({
    ...
    map2 <- leaflet() %>% 
      addTiles(options = providerTileOptions(noWrap = TRUE)) %>% 
      addCircleMarkers(lng=visit.x, lat=visit.y, group='circles',
  ...
  observeEvent(input$mapFollowing_marker_click,{
    ...
    userdes <- data.frame(cbind(ScreenName, Name, Description, Location, 
                                FriendsCount, StatusCount, FavoritesCount, 
                                AccountCreated))
    output$followingDescription <- renderDataTable({return(userdes)})
  })
  ...
  observeEvent(input$mapFollowing_marker_click,{
    ...
    for (i in 1:nrow(datsmall))
    {
      dat <- data.frame(cbind(twListToDF(userTimeline(datsmall$screenName[i], n = 10))$text, 
                              datsmall$screenName[i]))
      colnames(dat) <- c("Tweets", "ScreenName")
      dat2 <- dplyr::select(dat, ScreenName, Tweets)
      total <- rbind(total, dat2)
    }
    output$followingTweets <- renderDataTable({return(unique(total))})
  })
  
  output$downloadFollowing <- downloadHandler(
    filename = function(){paste('FollowingData', '.csv', sep = '')},
    content = function(file){
  ...
  observeEvent(input$mapFollower_marker_click,{
    ...
    userdes <- data.frame(cbind(ScreenName, Name, Description, Location, 
                                FriendsCount, StatusCount, FavoritesCount, 
                                AccountCreated))
    output$followerDescription <- renderDataTable({return(userdes)})
  })
  ...
  observeEvent(input$mapFollower_marker_click,{
    ...
    output$followerTweets <- renderDataTable({return(unique(total))})
  })
  
  output$downloadFollower <- downloadHandler(
    filename = function(){paste('FollowerData', '.csv', sep = '')},
    content = function(file){
  ...
  mapTweetsData <- eventReactive(input$showMapTweets, {
    ...
    incProgress(0.2, detail = "Finishing...")
    loc3 <- loc2 %>% dplyr::select(lon, lat, location, screenName, text, hour, weekday, month, year, 
  ...
  output$mapTweets <- renderLeaflet({
    withProgress(message = "Application loading", value = 0, {
      ...
      leaf <- leaflet() %>% 
        addTiles(options = providerTileOptions(noWrap = TRUE)) %>% 
        addCircleMarkers(lng=loc2$lon, lat=loc2$lat, 
                         group='circles', 
      ...
      {
        leaf <- leaf %>% addLegend(pal = factpal, values = (loc2[,var]),
                                   position = "bottomleft")
      }
      incProgress(0.2, detail = "Finishing...")
      return(leaf)
  ...
  observeEvent(input$mapTweets_marker_click,{
    ...
    datsmall <- subset(loc2, lat2 == lat1 & lon2 == lon1)
    datsmall <- datsmall[ ,!(colnames(datsmall) %in% c("lon2", "lat2"))]
    output$geolocatedTweets <- renderDataTable({return(datsmall)})
  })
  
  output$downloadTweetMapping <- downloadHandler(
    filename = function(){paste('TweetsData', '.csv', sep = '')},
    content = function(file){
...
output$followingNetworkUser <- renderDataTable({
  if (is.null(input$current_node_id3) == TRUE) 
  {
    ...
    user = getUser(toNode)
    Categories <- c("ScreenName", "Name", "Created", "Description", "Location", "FavoritesCount", 
                    "FollowersCount", "FriendsCount", "StatusesCount")
    Values <- c(as.character(user$screenName), as.character(user$name), 
...
emojiTrendsDataUser <- eventReactive(input$showEmojiAnalysisUser, {
  ...
  tweet43 <- unique(tab)
  text <- data.frame(text = iconv(tweet43$text, "latin1","ASCII", "byte"),
...
output$plotEmojiTrendsUser = renderPlotly({
  ...
  if (nrow(df.expanded) == 0)
  {
    ...
    small <- df.expanded %>% dplyr::group_by(dateC, Emoji) %>% dplyr::summarise(sum1 = sum(V1), 
                                                                                sumReT = sum(retweet3), sumFav = sum(favorite3)) %>% dplyr::arrange(dateC, desc(sum1), 
    ...
    if (nrow(small) != 0)
    {
      talDat <- small %>% dplyr::group_by(Emoji) %>% dplyr::summarise(EmojiCount = sum(NumTweets), RetweetCount = sum(NumReTweets), 
                                                                      FavCount = sum(NumFavorite)) %>% dplyr::arrange(desc(EmojiCount), desc(RetweetCount), desc(FavCount))
      ...
      talDat6 <- talDat5 %>% dplyr::select(Description, Emoji, EmojiCount, EmojiPercent, RetweetCount, RetweetPercent, FavCount,
                                           FavoritePercent)
      ...
      talDat7 <- melt(talDat6, id.vars = c("Description", 
                                           "Emoji", "EmojiCount", "RetweetCount", "FavCount"))
      
      talDat7$variable <- revalue(talDat7$variable, c("EmojiPercent" = "TweetCount", "RetweetPercent" = "RetweetCount", 
                                                      "FavoritePercent" = "FavoriteCount"))
      
      talDat7$variable <- revalue(talDat7$variable, c("TweetCount" = "TweetPercent", "RetweetCount" = "RetweetPercent", 
      ...
      ft <- ggplot(talDat7, aes(x = Emoji, y = value, group = variable, colour = variable, text = EmojiCount)) + geom_point(size = 3) + 
        ggtitle("Tracking Overall Emoji, Retweet and Favorite Percentage") + theme_bw() + theme(plot.margin=unit(c(1,1,1.5,1.2),"cm"),
...
output$emojiTrendsOverTimeUser <- renderDataTable({
  df.expanded <- emojiTrendsDataUser()
  if (nrow(df.expanded) == 0) {return(df.expanded)}
  else{
    small <- df.expanded %>% dplyr::group_by(dateC, Emoji) %>% dplyr::summarise(sum1 = sum(V1), 
                                                                                sumReT = sum(retweet3), sumFav = sum(favorite3)) %>% dplyr::arrange(dateC, desc(sum1), 
                                                                                                                                                    desc(sumReT), desc(sumFav))
